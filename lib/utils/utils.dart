...
class SHA256 {
  ...
  static String binaryToHex(List<int> bits) {
    ...
    for (int i = 0; i < binaryString.length; i += 4) {
      String nibble = binaryString
          .substring(i, min(i + 4, binaryString.length))
          .padLeft(4, '0');
  ...
  static List<int> fillZeros(List<int> bits,
      {int length = 8, String endian = 'LE'}) {
  ...
  static List<List<int>> preprocessMessage(String message) {
    ...
    List<int> length64 = int.parse(length.toRadixString(2))
        .toString()
        .padLeft(64, '0')
        .split('')
  ...
  static List<int> AND(List<int> i, List<int> j) =>
      List.generate(i.length, (index) => and_(i[index], j[index]));
  ...
  static List<int> XOR(List<int> i, List<int> j) =>
      List.generate(i.length, (index) => xor(i[index], j[index]));

  // if number of true values is odd return true
  static int xorxor(int i, int j, int l) => xor(i, xor(j, l));
  static List<int> XORXOR(List<int> i, List<int> j, List<int> l) =>
      List.generate(i.length, (index) => xorxor(i[index], j[index], l[index]));
  ...
  static int majority(int i, int j, int k) {
    ...
    return values.reduce((a, b) =>
        values.where((x) => x == a).length > values.where((x) => x == b).length
            ? a
            : b);
  }
  ...
  static String hash(String message) {
    // Initialize the constant round keys (K) and initial hash values (H)
    List<List<int>> k =
        initialiser(K); // 64 constant 32-bit words (used in each round)
    List<List<int>> hInitial =
        initialiser(H); // Initial 8 hash values (H0...H7)

    // Copy initial hash values so we can update them per chunk
    List<List<int>> h = [
      hInitial[0],
      hInitial[1],
      hInitial[2],
    ...
    // Preprocess the message into 512-bit chunks (with padding)
    ...
    for (var chunk in chunks) {
      ...
      // Extend the 16 words into 64 words (message schedule array)
      w.addAll(List.generate(48, (_) => List.filled(32, 0)));

      //  Message schedule extension (W[16..63])
      for (int i = 16; i < 64; i++) {
        List<int> s0 =
            XORXOR(rotr(w[i - 15], 7), rotr(w[i - 15], 18), lsr(w[i - 15], 3));
        List<int> s1 =
            XORXOR(rotr(w[i - 2], 17), rotr(w[i - 2], 19), lsr(w[i - 2], 10));
        w[i] = add(add(add(w[i - 16], s0), w[i - 7]), s1);
      }

      // Initialize working variables a-h with current hash state
      List<int> a = h[0];
      List<int> b = h[1];
      List<int> c = h[2];
      List<int> d = h[3];
      ...
      // Main compression function loop (64 rounds)
      for (int j = 0; j < 64; j++) {
        ...
        // Update working variables: h → g → f → ... → a
        hh = g;
        g = f;
        f = e;
        e = add(d, temp1);
      ...
      // Produce the final hash value (digest) by concatenating h0 to h7
    StringBuffer digest = StringBuffer();
    for (var val in h) {
      digest.write(binaryToHex(val)); // Convert binary array to hex string
    }

    return digest.toString(); // Return the final 256-bit hash in hex format
  }
}
